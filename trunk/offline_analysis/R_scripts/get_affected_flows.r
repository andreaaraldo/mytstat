...
host_proto_association[host_proto_association$ipaddr=="89.186.73.155",]
...
head( windows_temp1[windows_temp1$ipaddr1=="41.2.194.42",] )
...
protocol_annotated_windows[
  protocol_annotated_windows$ipaddr1=="41.2.194.42"&
  protocol_annotated_windows$ipaddr2=="89.186.73.155" &
  protocol_annotated_windows$port1==1302 &
  protocol_annotated_windows$port2==80 &
  protocol_annotated_windows$edge==1182430570
  ,]
...
point_C2S_2 <- 
  protocol_annotated_windows[
    protocol_annotated_windows$qd_samples_C2S>0,
    c("edge", "ipaddr1","port1","ipaddr2","port2",
      "protocol2", "windowed_qd_C2S")]

#Useful constants
windowed_qd_id <- c("edge", "ipaddr1","port1","ipaddr2","port2")
point_id <- c( windowed_qd_id, c("protocol"))
point_colnames <- c(point_id, c("windowed_qd"))
...
point_S2C_1 <- 
  protocol_annotated_windows[
    ...
    c("edge", "ipaddr1","port1","ipaddr2","port2",
      "protocol1", "windowed_qd_S2C")]

point_S2C_2 <- 
  protocol_annotated_windows[
  protocol_annotated_windows$qd_samples_S2C>0,
  c("edge", "ipaddr1","port1","ipaddr2","port2",
    "protocol2", "windowed_qd_S2C")]
...
head(windows_[windows_$ipaddr1=="41.2.194.42",] )
point[point$ipaddr1=="41.2.194.42" & point$edge==1182430570 &
      point$port1==1302 & point$ipaddr2=="89.186.73.155" &
      point$port2==80, ]
...
if( length(protocol_annotated_windows[is.na(protocol_annotated_windows$ipaddr1) | 
  protocol_annotated_windows[is.na(protocol_annotated_windows$ipaddr2) ,1])
)
  stop("ip addresses cannot be NA")

if( length( 
  point[duplicated(point[,c(windowed_qd_id,c("protocol")) ]),1]) 
    != 0
  )
    stop("Bizarre duplicates in point")
####### SEVERE DEBUG
