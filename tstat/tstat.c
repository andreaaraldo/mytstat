...
int ledbat_window= -1;  /* 0 means only w=0, 1 means only w=1, -1 means both */
...
#endif
    "\t --w0: compute biased queuing delay in LEDBAT (i.e. 1 measure = 1 sample)\n"
    "\t --w1: compute windowed queuing delay in LEDBAT (i.e. 1sec = 1 sample)\n"
...
*/
  ...
  //<aa>
  #ifdef SEVERE_DEBUG
  static Bool same_addresses_warned;
  #endif
  //</aa>
  ...
  do
	...
	if(	memcmp(&(pip->ip_src), &(pip->ip_dst), sizeof(pip->ip_dst) ) == 0 &&
		!same_addresses_warned
	){
		same_addresses_warned = TRUE;
		printf("tstat.c %d: WARNING: processing file %s: found at least one pkt with ip_dst(%s) == ip_src((%s))\n",
  ...
  while (1)
    {
      ...
      //if (debug > 2)
	fprintf (fp_stdout, "ParseArgs[%d]=%s\n option_index %d ", optind, argv[optind], option_index);
